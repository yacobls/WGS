...
oc,
          ...
          FROM oc
          WHERE status IN ('PENDING OC', 'APPROVED', 'SPK ISSUED')
          GROUP BY status, oc
          ORDER BY
            CASE
              WHEN status = 'PENDING OC' THEN 1
              WHEN status = 'APPROVED' THEN 2
              WHEN status = 'SPK ISSUED' THEN 3
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($row["status"] == 'PENDING OC') {
            echo "<a href='#' class='green-button' onclick='updateStatusOC(\"" . $row["oc"] . "\")'><i class='zmdi zmdi-check-circle' title='APPROVED' ></i></a>";
